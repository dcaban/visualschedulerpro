...
exports.buyStock = (req, res, next) => {
  ...
  User.findById(req.user._id, (err, user) => {
    if (err) {
      ...
      // Add stock to user's daily
      updateDaily(user.daily, transaction);
...
exports.sellStock = (req, res, next) => {
  ...
  User.findById(req.user._id, (err, user) => {
    if (err) {
      ...
      if (!hasEnoughShares(user.daily, stockSymbol, shares)) {
        return res.status(409).send({ message: "Invalid transaction" });
      }
...
// check if stock exists in daily
const containsStock = (daily, stockSymbol) => {
  return daily.some(stock => stock.stockSymbol === stockSymbol);
};

// update stock daily
const updateDaily = (daily, transaction) => {
  const { stockSymbol, stockName, action, shares } = transaction;

  // Check if user already own's stock
  if (containsStock(daily, stockSymbol)) {
    // updated number of shares for given stock
    updateStock(daily, stockSymbol, shares, action);
  } else if (action === "BUY") {
    ...
    daily.push(stockToAdd);
  }
};

// update daily when user owns stock already
const updateStock = (daily, stockSymbol, shares, action) => {
  daily.map(stock => {
    if (stock.stockSymbol === stockSymbol) {
      ...
      // Remove stock from daily if there are zero shares left
      if (totalShares === 0) {
        stock.remove();
      } else {
        // Otherwise update number of shares of stock in daily
...
// Check if user has enough shares
const hasEnoughShares = (daily, stockSymbol, shares) => {
  return daily.some(stock => {
